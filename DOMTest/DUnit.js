...
//
//   NOTE: 
//   We could write our own simple lib for defining straight jquery tests, and
//   adding them to the testQueue (as seen below)
//   OR we could use mocha or jasmine, or QUnit.
//
//=======================================================================================
var DUnit = DUnit || {};
(function (jQuery) {
    ...
    DUnit.ajaxStash = {};
    DUnit.wait_in_progress = 0;
    DUnit.wait_in_prog_func;

    DUnit.ajax_wait_in_progress = 0;
    DUnit.ajax_wait_in_prog_func;

    DUnit.testData = {}; // a place for comparing data 
    DUnit.testResults = {"passed":0, "failed":0,"total":0, tests:{}};

    //==================================================================
    // Add in a supporting JQ UI Highlighting function (for sex appeal)
    //==================================================================
    jQuery.fn.flash = function( color, duration ){
        var current = this.css( 'backgroundColor' );
        this.animate( { backgroundColor: color  }, duration / 4 );
        this.animate( { backgroundColor: current }, duration / 4 );
        this.animate( { backgroundColor: color  }, duration / 4 );
        this.animate( { backgroundColor: current }, duration / 4 );
    }

    //==================================================================
    // Add Selector refreshability
    //==================================================================
    jQuery.fn.extend({  
        refresh: function() { return jQuery(this.selector); }  
    });  

    //==================================================================
    // Plug IE9 for lack of console
    //==================================================================
    if(!window.console){console={}; console.log = function(){};}


    //==================================================================
    // Add a capture to check all post bodies for correct data
    //==================================================================
    jQuery( document ).ajaxSend(function( event, xhr, settings ) {
                                if(!DUnit.ajaxStash){return}
                                var postTarget = settings.url;
                                
                                var re = new RegExp(window.location.origin);
                                postTarget = postTarget.replace(re, "");
                               
                                DUnit.ajaxStash[postTarget] = {"status" : "started", "postBody": settings.data};
    ...
    jQuery( document ).ajaxComplete(function( event, xhr, settings ) {
                                ...
                                if(!DUnit.ajaxStash[postTarget]){return}
                                DUnit.ajaxStash[postTarget]["status"] = "completed";
    });    
    ...
    //==================================================================
    // Add a wait function for a DOM element to appear
    //  waitFor(selector, test, timeout, callback)
    //==================================================================
    DUnit.waitFor = function (selector, test, timeout, callback){

       //selectors may initially be undefined, so refresh them
       setTimeout(function () {
         var value;
         if (selector){  selector = selector.refresh(); value = selector.html(); }
         if (value && value.match(test)){
            ...
            // dequeue this
            //  if(wait_in_prog_func){clearTimeout(wait_in_prog_func);}
            //  var self = this;
            //  jQuery(self).dequeue("TestQueue");
            //  wait_in_progress = 0;
              jQuery(DUnit.wait_in_prog_func).dequeue("TestQueue");
              DUnit.wait_in_progress = 0;
            
 

         }else{
            DUnit.waitFor(selector, test, timeout, callback);
         }
       }, 1000); //check every second


       if(!DUnit.wait_in_progress){ //only queue this recursive wait once
          DUnit.wait_in_progress = 1;
          jQuery("body").queue("TestQueue", function() {
            //var self = this;
            DUnit.wait_in_prog_func = this;
            setTimeout(function() {
              jQuery(self).dequeue("TestQueue");
              DUnit.wait_in_progress = 0;
            }, timeout );  //note the timeout here is how long we wait in total
          });
       }
    };//End queued wait
    ...
    //==================================================================
    // Add a wait function AJAX calls
    //  waitForAjax(endpoint, timeout, callback)
    //==================================================================
    DUnit.waitForAjax = function (endpoint, timeout, callback){

       //selectors may initially be undefined, so refresh them
       setTimeout(function () {
         
         if ( DUnit.ajaxStash[endpoint] && DUnit.ajaxStash[endpoint]["status"]==="completed"){
            ...
            DUnit.waitForAjax(endpoint, timeout, callback);
         }
       }, 500); //check every half second


       if(!DUnit.ajax_wait_in_progress){ //only queue this recursive wait once
          DUnit.ajax_wait_in_progress = 1;
          jQuery("body").queue("TestQueue", function() {
            //var self = this;
            DUnit.ajax_wait_in_prog_func = this;
            setTimeout(function() {
              jQuery(DUnit.ajax_wait_in_prog_func).dequeue("TestQueue");
              DUnit.ajax_wait_in_progress = 0;
            }, timeout );  //note the timeout here is how long we wait in total
    ...
    //==================================================================
    //Add a func to queue a pause
    //==================================================================
    DUnit.pause = function (delay){
          jQuery("body").queue("TestQueue", function() {
            var self = this;
    ...
    //==================================================================
    //Add a func to queue a task (any task can do multiple things)
    //==================================================================
    DUnit.task = function (task_function, delay, callback){
          jQuery("body").queue("TestQueue", function() {
            task_function();
            var self = this;
            setTimeout(function() {
    ...
    //==================================================================
    //Add a func to evaluate a test (This is just for an example)
    //==================================================================
    DUnit.test = function (test, testName, callback){

          DUnit.testResults["total"]++;

          DUnit.testResults["tests"][testName] = {};
          DUnit.testResults["tests"][testName]["success"] = false;
          DUnit.testResults["tests"][testName]["failure"] = false;
          DUnit.testResults["tests"][testName]["error"] = '';

          if(test){
             console.log("Passed: " + testName);
             DUnit.testResults["tests"][testName]["success"] = true;
             DUnit.testResults["passed"]++;
          }else{
             console.log("Failed: " + testName);
             DUnit.testResults["tests"][testName]["failure"] = true;
             DUnit.testResults["tests"][testName]["error"] = '';
             DUnit.testResults["failed"]++;
          }
    ...
    //======================================
    // Post results back to server when done
    //======================================
    DUnit.post_results = function () {

          var testStatus = "failed";
          if(DUnit.testResults["passed"] === DUnit.testResults["total"]){
              testStatus = "success";
          }

          // Post Back from whence you came
          var postBackURL = window.location.protocol + "//" + window.location.host +"/test_results";
          ...
          jQuery.ajax({
            ...
            success: function(){ 
              console.log("Testing Complete; Results Posted.");
            },
            dataType: 'text'
          });
    ...
    //============================================================
    // Take a screenshot with either vbox or html2canvas
    //============================================================
    DUnit.screenshot = function (filename){

        //=====================================
        // True Images with vBox
        //=====================================
        DUnit.task(function(){
           var postBackURL = window.location.protocol + "//" + window.location.host +"/vbox_snapshot";
           jQuery.post(postBackURL, { 'filename': filename });
        }, 500);
        DUnit.waitForAjax("/vbox_snapshot", 10000, function(data){
            console.log("Snapshot is complete");
        });
        

        //=====================================
        // Client Side Images with html2canvas
        //=====================================
        //  DUnit.task(function(){
        //    var postBackURL = window.location.protocol + "//" + window.location.host +"/save_clientside_snapshot";
        //    var targetElem = jQuery(document.body);
        //          html2canvas(targetElem[0], {
        //              useCORS: false
        //              }).then(
        //                  function(canvas) {
        //                  var imgdata = canvas.toDataURL('image/png'); 
        //                  jQuery.post(postBackURL, 
        //                              { 'filename': filename, 
        //                                'image': imgdata }).done(function(data) {
        //                                                        //console.log('Snaphot Complete');
        //                                                    });
        //              }
        //          );
        //  }, 500);
        //  DUnit.waitForAjax("/save_clientside_snapshot", 10000, function(data){
        //   console.log("Snapshot is complete");
        //  });
    ...
    //============================================================
    // Lastly queue the results post and cleanup the testQueue!!!
    //============================================================
    DUnit.done = function (){
